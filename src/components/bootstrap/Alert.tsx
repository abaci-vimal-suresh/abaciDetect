...
interface IAlertProps extends Record<string, any> {
	...
	isGlass?: boolean; // New glassmorphic style
	isNeumorphic?: boolean; // New neumorphic style
}
const Alert: FC<IAlertProps> = ({
	...
	const handleClose = () => {
		setIsClosing(true);
		setTimeout(() => setStatus(false), 300); // Match animation duration
	};
		...
		return (
			<div
				className={classNames(
					...
					'alert-modern',
					{
						[`alert-${color}`]: color && !(isLight || isOutline || isGlass || isNeumorphic),
						'alert-dismissible': isDismissible,
						'alert-glass': isGlass,
						'alert-neumorphic': isNeumorphic,
						'alert-closing': isClosing,
						fade: isDismissible,
						show: isDismissible,
						[`alert-light-${color}`]: isLight && !isGlass && !isNeumorphic,
						[`alert-outline-${color}`]: isOutline,
						[`shadow${shadow !== 'md' ? `-${shadow}` : ''}`]:
							!!shadow && shadow !== '3d' && !isGlass && !isNeumorphic,
						[`border-${borderWidth}`]: borderWidth || borderWidth === 0,
						[`rounded${rounded !== 'default' ? `-${rounded}` : ''}`]:
							rounded || rounded === 0,
						[`shadow-3d-${color}`]: shadow === '3d',
						[`alert-glass-${color}`]: isGlass && color,
						[`alert-neumorphic-${color}`]: isNeumorphic && color,
					},
					className,
				)}
				// eslint-disable-next-line react/jsx-props-no-spreading
				...
				{isDismissible && (
					<button
						type='button'
						className='btn-close btn-close-modern'
						aria-label='Close'
						onClick={handleClose}
					/>
				)}
