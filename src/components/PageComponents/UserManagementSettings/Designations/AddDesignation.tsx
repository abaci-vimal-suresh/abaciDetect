...
const AddDesignation = ({ isOpen, setIsOpen, title }) => {
    const {
        register,
        handleSubmit,
        control,
    ...
    return (
        <Modal isOpen={isOpen} setIsOpen={setIsOpen} size='lg' isCentered>
            <ModalHeader className='p-4' setIsOpen={setIsOpen}>
                <ModalTitle id='modalAddDesignation'>{title}</ModalTitle>
            </ModalHeader>
            <ModalBody className='d-flex flex-column gap-4 px-5 pb-5'>
                {/* Designation Name */}
                <FormGroup label="Designation Name">
                    <input
                        type="text"
                        className={errors?.designation_name?.type === "required" ? 'form-control is-invalid' : 'form-control'}
                        placeholder="Enter designation name"
                        {...register("designation_name", { required: true })}
                    />
                    {renderError('designation_name')}
                </FormGroup>
