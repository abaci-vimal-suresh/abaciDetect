...
export function FloorModel({
    ...
    useEffect(() => {
        if (!calibrated) {
            if (isImage && texture) {
                // Determine aspect ratio from texture image
                const img = texture.image;
                const aspect = img ? img.width / img.height : 1;

                // For images, we use a default "Large" size to ensure sensors fit
                // Matches FloorImagePlane logic
                const IMAGE_SIZE = 30;
                const width = IMAGE_SIZE;
                const depth = IMAGE_SIZE / aspect;
                const IMAGE_HEIGHT = 2.4; // standard room height

                const size = new THREE.Vector3(width, IMAGE_HEIGHT, depth);
                const min = new THREE.Vector3(-width / 2, 0, -depth / 2);
                ...
                if (onLoad) {
                    onLoad({
                        ...
                        depth: size.z,
                        height: size.y,
                        minX: centerModel ? -size.x / 2 : min.x,
                        minZ: centerModel ? -size.z / 2 : min.z,
                        minY: min.y,
                        centerX: centerModel ? 0 : size.x / 2,
                        centerZ: centerModel ? 0 : size.z / 2,
                        isFallback: true,
                        isReady: true
                    });
                }
                ...
                if (onLoad) {
                    console.log(`[FloorModel:onLoad] areaId(${areaId}) calibration:`, {
                        width: size.x,
                        depth: size.z,
                        height: size.y,
                        minX: centerModel ? -size.x / 2 : box.min.x,
                        minZ: centerModel ? -size.z / 2 : box.min.z,
                        centerX: centerModel ? 0 : center.x,
                        centerZ: centerModel ? 0 : center.z,
                        isReady: true
                    });
                    onLoad({
                        width: size.x,
                        depth: size.z,
                        height: size.y,
                        // Fix: if centered, min bounds are exactly half the size
                        minX: centerModel ? -size.x / 2 : box.min.x,
                        minZ: centerModel ? -size.z / 2 : box.min.z,
                        minY: box.min.y,
                        centerX: centerModel ? 0 : center.x,
            ...
            {isImage ? (
                ...
                <primitive
                    object={(() => {
                        ...
                        // Propagate areaId to ALL descendant meshes so raycasting
                        // always finds it regardless of which child is hit
                        scene.traverse((node: any) => {
                            node.userData = { ...node.userData, areaId };
                        });
                        return scene;
                    })()}
